...
package object barneshut {
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      if (b.x >= (centerX - size/2) && b.x < centerX &&  b.y >= (centerY - size/2) && b.y < centerY)
        Fork(nw.insert(b), ne, sw, se)
      else if (b.x >= centerX  && b.x < (centerX + size/2) &&  b.y >= (centerY - size/2) && b.y < centerY)
        Fork(nw, ne.insert(b), sw, se)
      else if (b.x >= (centerX - size/2) && b.x < centerX &&  b.y >= centerY && b.y < (centerY + size/2))
  ...
  extends Quad {
    val mass = bodies.foldLeft(0.0f)((m,b) => m + b.mass)
    val massX = bodies.foldLeft(0.0f)((mx,b) => mx + b.mass * b.x) / mass
    val massY = bodies.foldLeft(0.0f)((my,b) => my + b.mass * b.y) / mass
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def wrap(x:Float, min:Float, max:Float):Float =
        if (x < min) min
        else if (x > max) max
        else x
      this(((wrap(b.x, boundaries.minX, boundaries.maxX) - boundaries.minX)/sectorSize).toInt,
        ((wrap(b.y, boundaries.minY, boundaries.maxY) - boundaries.minY)/sectorSize).toInt) += b
      this
    }
